...
public class IfCommand extends AbstractCommand {
    ...
    public void execute(Map<String, String> variables, WorkSpace workSpace, Map<String, String> arguments) {
        String value = variables.get(arguments.get(CONDITION));
        if (value != null) {
            value = value.trim();
        }
        boolean confitionFalse = (value == null) || value.isEmpty() || value.equals(ZERO) || value.equalsIgnoreCase(NO) || value.equalsIgnoreCase(FALSE);
        ...
        } else if (arguments.containsKey(VALUE1)) {
            variables.put(arguments.get(VARIABLE), arguments.get(confitionFalse ? VALUE2 : VALUE1));
        } else if (!confitionFalse) {
